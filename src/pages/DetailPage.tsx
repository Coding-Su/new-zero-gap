...
const DetailPage = () => {
  ...
  const handleShowSourceLog = async () => {
    if (!feature?.id) return;
    setIsLoading(true);
    try {
      const log = await fetchMeetingLogByFeatureId(feature.id);
      if (log) {
        setSourceLog(log);
  ...
  return (
    <div className="detail-page-root">
      <header className="detail-header">
        ...
        <button onClick={handleShowSourceLog} className="backtrace-button">
          <FileText size={18} />
          <span>도출 근거(회의록)</span>
        </button>
      </header>
        ...
        <div className="zg-modal-overlay" onClick={() => setIsLogModalOpen(false)}>
          <div className="zg-modal-content detail-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="zg-modal-body">
              <div className="log-meta"><span><strong>일시:</strong> {sourceLog.createdAt}</span><span><strong>작성자:</strong> {sourceLog.author}</span></div>
